package alg77777777.s6.course2013.Sukodu;
/* SUDOKU
 * Solves a Sudoku (9x9 that is in a file)
 * The name of the file (without extension) is arg[0]
 * Calculates one solution and finish the process */
...
public class SudokuOne {
	...
	static boolean wasFound;//whether a solution was found (true or false)
	...
	public static void main(String arg[]) {
		...
		if (!wasFound) System.out.println("THERE IS NO SOLUTION");
	}
	...
	static void backtracking(int x, int y) {
		if (x==n) { //we found a solution (AND FINISH)
			...
			for (int k=1; k<=n; k++) { //from 1 to n
				if(!wasFound && row(x,k) && column(y,k) && region(x,y,k)) {
					board[x][y] = k; //mark
